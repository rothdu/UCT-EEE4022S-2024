...
def processData(file_hdf5, crop=True):
    ...
    # print(f"Data read time: {(end_time - start_time)*1e3}")
    ...
    # print(f"Data processing time: {(end_time - start_time)*1e3}")
...
class GestureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            data_cube = radarDataTensor(file_hdf5)
...
def runCnn():
    classes = ("virtual_tap","virtual_slider_left","virtual_slider_right","swipe_left","swipe_right", "swipe_up", "swipe_down",
            "palm_grab", "palm_release", "virtual_knob_clockwise", "virtual_knob_anticlockwise", "pinch_out_horizontal", 
            "pinch_out_vertical", "non_gesture")
    ...
    dataset = GestureDataset(input_csv, root_dir, classes, transform=data_transforms)
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset)):
        ...
        if fold == 1: # just run the first 3 folds for an initial sense
            break
...
def evaluate(model, loader, criterion):
    ...
    with torch.no_grad(): # evaluating so don't produce gradients
        for data in loader:
            ...
            # start_time = time.time()
            # print("Data transfer time:", (time.time() - start_time)*1000)
